...
static int mspm0_erase(struct flash_bank *bank, unsigned int first, unsigned int last)
{
	...
	/*
	 * Depending on the content from the image being used. Parameter's
	 * first and last "can" or "cannot" be sufficient enough for us to
	 * determine what to erase. This is why we first check the base of
	 * what we wish to erase. If the image contains content from both 
	 * NONMAIN and MAIN for example, then both first and last will return
	 * 0 which can tell us to perform a mass erase. However, this is 
	 * not enough information to tell us whether or not NONMAIN should
	 * be erased as well. Through checking the base of the bank being
	 * passed in we can more accurately determine what region of memory
	 * needs to be erased.
	 */
	switch(bank->base)
	{
		...
		{
			/*
			 * NONMAIN is only a sector (1kB) in size. In the case the bank
			 * passed in is NONMAIN, we will protect all of MAIN memory and
			 * perform a sector erase only on NONMAIN memory.
			 */
			LOG_DEBUG("%s: Erasing NONMAIN",mspm0_info->name);
			target_write_u32(target, FCTL_REG_CMDWEPROTA, (uint32_t)0xFFFFFFFF); //unprotecting first 32kB of MAIN
			target_write_u32(target, FCTL_REG_CMDWEPROTB, (uint32_t)0xFFFFFFFF); //unprotecting remaining sectors of MAIN
			target_write_u32(target, FCTL_REG_CMDWEPROTNM, (uint32_t)0x00000000); //protecting all of NONMAIN
			target_write_u32(target, FCTL_REG_CMDTYPE, (FCTL_CMDTYPE_COMMAND_ERASE | FCTL_CMDTYPE_SIZE_SECTOR)); //setting erase to be size of sector
			target_write_u32(target, FCTL_REG_CMDADDR, MSPM0_FLASH_BASE_NONMAIN);
			target_write_u32(target, FCTL_REG_CMDEXEC, FCTL_CMDEXEC_VAL_EXECUTE);
			retval = msmp0_fctl_wait_cmd_ok(bank);
			if (retval) {
				LOG_ERROR("%s: Failed performing a sector erase on NONMAIN", mspm0_info->name);
				return retval;
				}
			break;
		}
		case MSPM0_FLASH_BASE_MAIN:
		{
			LOG_DEBUG("%s: Erasing MAIN",mspm0_info->name);
			if((last == first)){ //perform a mass erase
				LOG_DEBUG("%s: Performing mass erase", mspm0_info->name);
				if(mspm0_info->main_flash_num_banks > 1){
					uint32_t exec_upper_bank;
					uint32_t sysctl_sec_status;
					...
					uint8_t bankIndex = 0;
					uint8_t eraseFlag = 0;
					uint32_t bankSize = ((mspm0_info->main_flash_size_kb / mspm0_info->main_flash_num_banks) * mspm0_info->sector_size);

					while (bankIndex < mspm0_info->main_flash_num_banks) {
						/*
						* Before performing the bank erase a check is done to 
						* determine if the flash banks have been swapped. If swapped,
						* the primary bank will be write and erase protected so we
						* will not perform any operation on it.
						*/
						target_read_u32(target, SYSCTL_SECCFG_SECSTATUS, &sysctl_sec_status);
						exec_upper_bank = mspm0_extract_val(sysctl_sec_status, 12, 12);
						if(exec_upper_bank){
							eraseFlag = (bankIndex < (mspm0_info->main_flash_num_banks / (uint32_t)2)) ? 1 : 0;
						} else{
							eraseFlag = 1;
						}
						...
						if(eraseFlag == 1){
							...
							target_write_u32(target, FCTL_REG_CMDWEPROTA, (uint32_t)0x00000000); //unprotecting first 32kB of MAIN
							target_write_u32(target, FCTL_REG_CMDWEPROTB, (uint32_t)0x00000000); //unprotecting remaining sectors of MAIN
							target_write_u32(target, FCTL_REG_CMDWEPROTNM, (uint32_t)0xFFFFFFFF); //protecting all of NONMAIN
							target_write_u32(target, FCTL_REG_CMDTYPE, (FCTL_CMDTYPE_COMMAND_ERASE | FCTL_CMDTYPE_SIZE_BANK)); //setting erase to be size of bank
							target_write_u32(target, FCTL_REG_CMDADDR, bankStartAddr);
							target_write_u32(target, FCTL_REG_CMDEXEC, FCTL_CMDEXEC_VAL_EXECUTE);
							retval = msmp0_fctl_wait_cmd_ok(bank);
							if (retval) {
								LOG_ERROR("%s: Failed performing mass erase on bank %d", mspm0_info->name, bankIndex);
								return retval;
							}
						}
						bankIndex++;
					...
					target_write_u32(target, FCTL_REG_CMDWEPROTNM, (uint32_t)0xFFFFFFFF); //protecting all of NONMAIN
					target_write_u32(target, FCTL_REG_CMDTYPE, (FCTL_CMDTYPE_COMMAND_ERASE | FCTL_CMDTYPE_SIZE_BANK)); //setting erase to be size of bank
					target_write_u32(target, FCTL_REG_CMDADDR, (uint32_t)0x00000000);
					target_write_u32(target, FCTL_REG_CMDEXEC, FCTL_CMDEXEC_VAL_EXECUTE);
					retval = msmp0_fctl_wait_cmd_ok(bank);
					if (retval) {
						LOG_ERROR("%s: Failed performing mass erase", mspm0_info->name);
						return retval;
					}
				}
			} else{ //perform a sector erase
				LOG_DEBUG("%s: Performing a sector erase", mspm0_info->name);
				for (uint32_t csa = first; csa < last; csa++) {
					...
					retval = msmp0_fctl_wait_cmd_ok(bank);
					if (retval) {
						LOG_ERROR("%s: Failed performing sector erase at address 0x%08" PRIx32
							"(sector: %d)", mspm0_info->name, addr, csa);
						return retval;
					}
				}
		...
		case MSPM0_FLASH_BASE_DATA:
		{
			LOG_INFO("%s, Data region currently not supported",mspm0_info->name);
			break;
		}
		default:
			LOG_ERROR("%s: Invalid memory region",mspm0_info->name);
			break;
	}
		       ...
		       uint32_t offset, uint32_t count)
{
	...
	/*
	 * When an image is passed in and contains NONMAIN content.
	 * The offset that will be passed into the mspm0_write()
	 * function when beginning writes to NONMAIN will be 0x00. 
	 * Due to this, a check must be done before any write operation
	 * to ensure the offset is actually at NONMAIN instead of MAIN
	 * memory.
	 */
	if (bank->base == MSPM0_FLASH_BASE_NONMAIN){
	...
	while (count) {
		...
		LOG_DEBUG("%s: Writing to address : 0x%"PRIx32 , mspm0_info->name, offset);
